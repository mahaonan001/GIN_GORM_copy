...
func Register(c *gin.Context) {
	...
	//数据验证
	if len(Phone) != 11 {
		response.Response(c, http.StatusUnprocessableEntity, 422, nil, "手机号必须是11位数")
		return
	}
	if len(PassWord) < 6 {
		response.Response(c, http.StatusUnprocessableEntity, 422, nil, "密码不能低于6位数")
		return
	}
	if len(Name) == 0 {
	...
	if emailRegex.MatchString(Email) {
		...
		if isPhoneExited(db_student, Phone).ID != 0 {
			response.Response(c, http.StatusUnprocessableEntity, 422, nil, "该手机号已注册")
			return
		}
		DB_code := common.GetDB_Email()
		var Code_email model.EmailCode
		DB_code.Where("email=?", Email).Last(&Code_email)
		log.Println("flag:", mail.TimeInner(DB_code, Email))
		if !mail.TimeInner(DB_code, Email) { //验证码不可用
			response.Response(c, http.StatusServiceUnavailable, 402, nil, "验证码过期")
		} else if Code_Email == Code_email.Code_email { //存在符合条件的验证码
			...
			response.Response(c, http.StatusUnauthorized, 402, gin.H{"msg": "验证码填写错误"}, "err")
		}
	}
}
...
func Login(c *gin.Context) {
	...
	if user.ID == 0 {
		response.Response(c, http.StatusUnprocessableEntity, 402, nil, "用户不存在")
		return
	} else if user.ErrorTimes >= 3 {
		response.Response(c, http.StatusUnprocessableEntity, 422, nil, "密码错误次数过多，账号已冻结")
		return
	}
	//验证密码
...
func Info(ctx *gin.Context) {
	user, _ := ctx.Get("user")
	//ctx.JSON(http.StatusOK, gin.H{"code": 200, "data": gin.H{"user": dto.ToDto(user.(model.User))}})
